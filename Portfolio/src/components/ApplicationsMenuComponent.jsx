    ...
    <div className="bg-ubuntu-dock/90 h-screen w-24 opacity-65 absolute z-1 border-1 border-solid border-[#464646] flex flex-col py-4">
      <ul className="flex flex-col items-center grow-[0.90]">
        {applications.map((application, index) => (
          <li key={index} className="mb-2 rounded-xl p-3 hover:bg-gray-50/10 relative group cursor-pointer" onClick={handleAppIconClick} data-value={application.dataValue}>
            {application.icon ? (
              <application.icon className="w-14 h-14"/>
            ) : (
              <img src={application.imgSrc} alt={application.imgAlt} className="w-14 h-14"/>
            )}
